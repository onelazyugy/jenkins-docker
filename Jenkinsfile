    ...
    stages {
        ...
        stage('PUSH TO DOCKERHUB') {
            steps {
                ...
                sh 'docker login'
                sh 'docker tag jenkins onelazyguy/jenkins:0.0.3-SNAPSHOT'
                sh 'docker image ls'
                sh 'docker push onelazyguy/jenkins:0.0.3-SNAPSHOT'
            }
        }
        stage('DEPLOY APP') {
            steps {
                echo 'deploying app...'
                sh 'docker run -p 5001:9002 onelazyguy/jenkins:0.0.3-SNAPSHOT'
            }
        }
    }
}